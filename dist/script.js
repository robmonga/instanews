...
$(function () {
  ...
  $("#sections").on("change", function () {
    var section = $(this).val(); // if value is empty then return
    // show loader

    $("loader").show(); // clear stories

    $(".articles").empty(); // make our ajax request

    $.ajax({
      method: "GET",
      url: "https://api.nytimes.com/svc/topstories/v2/" + section + ".json?api-key=ioTcOVt0KS3240rgA0itgaAjvvXAkWZY",
      dataType: "json"
    }).done(function (data) {
      var articles = data.results.filter(function (result) {
        return result.multimedia[4] !== undefined;
      }).slice(0, 12);
      console.log(articles);
      $.each(articles, function (key, value) {
        var abstract = value.abstract;
        var media = value.multimedia[4].url;
        var url = value.url;
        $(".articles").append("<a target='_blank' href=" + url + " style='background-image:url(" + media + ")'><p>" + abstract + "</p></a>"); // append all the things
      }) //
      .fail(function () {
        $(".articles").append("Sorry, please refresh."); //do stuff here if it doesn't work out
      }) // hide the loader
      .always(function () {
        $('#loader').hide();
      });
